...
ZLSymbianDialog::ZLSymbianDialog(const ZLResource &resource) {
/*	myTab = new ZLGtkDialogContent(resource);
	myDialog = createGtkDialog(resource[ZLDialogManager::DIALOG_TITLE].value());
	myIsPacked = false;*/
}

ZLSymbianDialog::~ZLSymbianDialog() {/*
	gtk_widget_destroy(GTK_WIDGET(myDialog));*/
}

void ZLSymbianDialog::addButton(const ZLResourceKey &key, bool accept) {
...
/*	if (!myIsPacked) {
	...
	return gtk_dialog_run(GTK_DIALOG(myDialog)) == GTK_RESPONSE_ACCEPT;*/ return TRUE;
}
