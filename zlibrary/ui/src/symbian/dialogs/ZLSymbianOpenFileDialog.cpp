...
*/
ZLSymbianOpenFileDialog::ZLSymbianOpenFileDialog(const std::string &title, const std::string &directoryPath, const std::string &filePath, const Filter &filter) {
...
ZLSymbianOpenFileDialog::~ZLSymbianOpenFileDialog() {
/*	gtk_widget_destroy(GTK_WIDGET(myDialog));*/
}

bool ZLSymbianOpenFileDialog::run() {
/*	const int code = gtk_dialog_run(GTK_DIALOG(myDialog));
	gtk_widget_hide(GTK_WIDGET(myDialog));
	return code == GTK_RESPONSE_OK;*/return TRUE;
}

std::string ZLSymbianOpenFileDialog::filePath() const {
/*	const char *path = gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(myDialog));
	return (path != 0) ? path : std::string();*/return NULL;
}

std::string ZLSymbianOpenFileDialog::directoryPath() const {
