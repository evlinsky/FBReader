...
ZLSymbianOptionsDialog::ZLSymbianOptionsDialog(const ZLResource &resource, shared_ptr<ZLRunnable> applyAction) : ZLOptionsDialog(resource, applyAction) {
...
ZLSymbianOptionsDialog::~ZLSymbianOptionsDialog() {
/*	gtk_widget_destroy(GTK_WIDGET(myDialog));*/
}

ZLDialogContent &ZLSymbianOptionsDialog::createTab(const ZLResourceKey &key) {
	ZLSymbianDialogContent *tab = new ZLSymbianDialogContent(tabResource(key));
...
const std::string &ZLSymbianOptionsDialog::selectedTabKey() const {
	//return myTabs[gtk_notebook_get_current_page(myNotebook)]->key();
  return "";
}

void ZLSymbianOptionsDialog::selectTab(const ZLResourceKey &key) {
...
int ZLSymbianOptionsDialog::height() const {
/*	int _width, _height;

	gtk_window_get_size(GTK_WINDOW(myDialog), &_width, &_height);

	return _height;*/return 0;
}
