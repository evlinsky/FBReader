...
/*
ZLSymbianProgressDialog::ZLSymbianProgressDialog(GtkWindow *window, const ZLResourceKey &key) : ZLProgressDialog(key), myParent(window), myLabel(0) {
}
*/
void ZLSymbianProgressDialog::setMessage(const std::string &message) {
/*	if (myLabel == 0) {
		return;
	}
...
void ZLSymbianProgressDialog::run(ZLRunnable &runnable) {
/*	while (gtk_events_pending()) {
		gtk_main_iteration();
	}
