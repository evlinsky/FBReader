...
ZLSymbianCommunicationManager::ZLSymbianCommunicationManager()/* : myContext(0) */{
}
/*
...
*//*
gint ZLSymbianCommunicationManager::onMessageReceived(const gchar*, const gchar *method, GArray *arguments, osso_rpc_t*) {
	if (method != 0) {
...
*/
void ZLSymbianCommunicationManager::init() {/*
...
void ZLSymbianCommunicationManager::shutdown() {
...
void ZLSymbianCommunicationManager::addInputMessageDescription(const std::string &command, const std::string &protocol, const Data &data) {/*
	if (protocol == "osso-rpc") {
...
/*
ZLSymbianRpcMessageOutputChannel::ZLSymbianRpcMessageOutputChannel(osso_context_t *&context) : myContext(context) {
}
*/
shared_ptr<ZLMessageSender> ZLSymbianRpcMessageOutputChannel::createSender(const ZLCommunicationManager::Data &data) {
/*	ZLCommunicationManager::Data::const_iterator it = data.find("service");
	...
	return (!service.empty() && !command.empty()) ? new ZLMaemoRpcMessageSender(myContext, service, command) : 0;*/return 0;
}
/*
ZLSymbianPresentMessageOutputChannel::ZLSymbianPresentMessageOutputChannel(osso_context_t *&context) : ZLSymbianRpcMessageOutputChannel(context) {
}
*/
shared_ptr<ZLMessageSender> ZLSymbianPresentMessageOutputChannel::createSender(const ZLCommunicationManager::Data &data) {/*
	ZLCommunicationManager::Data copy = data;
	copy["service"] = copy["toPresent"];
	copy["command"] = "present";
	return ZLMaemoRpcMessageOutputChannel::createSender(copy);*/return 0;
}
/*
ZLSymbianRpcMessageSender::ZLSymbianRpcMessageSender(osso_context_t *&context, const std::string &service, const std::string &command) : myContext(context), myService(service), myCommand(command) {
}
*/
void ZLSymbianRpcMessageSender::sendStringMessage(const std::string &message) {
