...
*/
void ZLSymbianTimeManager::addTask(shared_ptr<ZLRunnable> task, int interval) {
/*	if ((interval > 0) && !task.isNull()) {
		myHandlers[task] = g_timeout_add(interval, (GSourceFunc)taskFunction, &*task);
	}*/
}

void ZLSymbianTimeManager::removeTaskInternal(shared_ptr<ZLRunnable> task) {
