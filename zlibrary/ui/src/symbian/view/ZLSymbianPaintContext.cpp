...
*/
ZLSymbianPaintContext::ZLSymbianPaintContext() {
/*	myPixmap = 0;
	myWidth = 0;
	myHeight = 0;
...
/*
void ZLSymbianPaintContext::updatePixmap(GtkWidget *area, int w, int h) {
...
*/
void ZLSymbianPaintContext::fillFamiliesList(std::vector<std::string> &families) const {
/*	if (myContext != 0) {
...
const std::string ZLSymbianPaintContext::realFontFamilyName(std::string &fontFamily) const {
/*	if (myContext == 0) {
		return fontFamily;
	}
...
void ZLSymbianPaintContext::setFont(const std::string &family, int size, bool bold, bool italic) {
...
void ZLSymbianPaintContext::setColor(ZLColor color, LineStyle style) {
...
void ZLSymbianPaintContext::setFillColor(ZLColor color, FillStyle style) {
/*	if (style == SOLID_FILL) {
...
int ZLSymbianPaintContext::stringWidth(const char *str, int len, bool rtl) const {
/*	if (myContext == 0) {
		return 0;
	}
	...
	return (logicalRectangle.width + PANGO_SCALE / 2) / PANGO_SCALE;*/return 0;
}

int ZLSymbianPaintContext::spaceWidth() const {
/*	if (mySpaceWidth == -1) {
		mySpaceWidth = stringWidth(" ", 1, false);
	}
	return mySpaceWidth;*/return 0;
}

int ZLSymbianPaintContext::stringHeight() const {
/*	if (myFontDescription == 0) {
		return 0;
	}
	if (myStringHeight == -1) {
	...
	return myStringHeight;*/return 0;
}

int ZLSymbianPaintContext::descent() const {
	return myDescent;
}

void ZLSymbianPaintContext::drawString(int x, int y, const char *str, int len, bool rtl) {
/*	if (!g_utf8_validate(str, len, 0)) {
		return;
	}
...
void ZLSymbianPaintContext::drawImage(int x, int y, const ZLImageData &image) {
...
void ZLSymbianPaintContext::drawImage(int x, int y, const ZLImageData &image, int width, int height, ScalingType type) {
...
void ZLSymbianPaintContext::drawLine(int x0, int y0, int x1, int y1) {
/*	gdk_draw_line(myPixmap, myTextGC, x0, y0, x1, y1);*/
}

void ZLSymbianPaintContext::fillRectangle(int x0, int y0, int x1, int y1) {
/*	if (x1 < x0) {
		int tmp = x1;
		x1 = x0;
		x0 = tmp;
...
void ZLSymbianPaintContext::drawFilledCircle(int x, int y, int r) {
...
void ZLSymbianPaintContext::clear(ZLColor color) {
/*	myBackColor = color;
	if (myPixmap != 0) {
...
int ZLSymbianPaintContext::width() const {
/*	if (myPixmap == 0) {
		return 0;
	}
	return myWidth;*/return 0;
}

int ZLSymbianPaintContext::height() const {
/*	if (myPixmap == 0) {
		return 0;
	}
	return myHeight;*/return 0;
}
